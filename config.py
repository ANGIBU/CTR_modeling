...
class Config:
    ...
    GPU_MEMORY_LIMIT = 10  # RTX 4060 Ti 16GB 중 10GB 사용 (보수적으로)
    CUDA_VISIBLE_DEVICES = "0"
    USE_MIXED_PRECISION = True
    
    # 메모리 최적화 설정 (64GB RAM 기준, 매우 보수적으로)
    MAX_MEMORY_GB = 20  # 64GB 중 20GB 사용 (더욱 보수적)
    CHUNK_SIZE = 200000  # 청킹 크기 더 감소
    BATCH_SIZE_GPU = 2048  # GPU 배치 크기 감소
    BATCH_SIZE_CPU = 512  # CPU 배치 크기 감소
    PREFETCH_FACTOR = 1  # 프리페치 팩터 최소화
    
    # 데이터 처리 전략 (매우 보수적으로)
    TARGET_DATA_USAGE_RATIO = 0.05  # 전체 데이터의 5% 목표
    MIN_TRAIN_SIZE = 300000  # 최소 학습 데이터 크기 감소
    MAX_TRAIN_SIZE = 600000  # 최대 학습 데이터 크기 감소
    MIN_TEST_SIZE = 50000  # 최소 테스트 데이터 크기 감소
    MAX_TEST_SIZE = 100000  # 최대 테스트 데이터 크기 감소
    ...
    LGBM_PARAMS = {
        ...
        'n_estimators': 1000,  # 더 감소
        'early_stopping_rounds': 100,
        'scale_pos_weight': 49.0,
        'force_row_wise': True,
        'max_bin': 128,  # 메모리 절약
        'num_threads': 4  # 스레드 수 제한
    }
    ...
    CAT_PARAMS = {
        ...
        'thread_count': 4  # 스레드 수 제한
    }
    
    # 딥러닝 모델 파라미터 (메모리 최적화)
    NN_PARAMS = {
        'hidden_dims': [256, 128, 64],  # 레이어 크기 더 감소
        'dropout_rate': 0.4,
    ...
    CALIBRATION_CONFIG = {
        ...
        'cv_folds': 3,
        'calibration_sample_size': 20000  # 메모리 절약
    }
    ...
    FEATURE_CONFIG = {
        ...
        'interaction_features': False,  # 메모리 절약
        'time_features': True,
        'statistical_features': False,  # 메모리 절약
        'preserve_ids': True,
        'id_hash_features': True,
        'polynomial_features': False,  # 메모리 절약
    ...
    # 실시간 추론 설정 (메모리 최적화)
    INFERENCE_CONFIG = {
        'batch_size': 2000,  # 배치 크기 감소
        'timeout': 100,
        'cache_size': 10000,  # 캐시 크기 감소
        'model_version': 'v2.0',
    ...
    TUNING_CONFIG = {
        'n_trials': 20,  # 트라이얼 수 감소
        'timeout': 1800,  # 타임아웃 감소
        'parallel_jobs': 1,  # 병렬 작업 수 최소화
        'pruner': 'MedianPruner',
        'sampler': 'TPESampler'
    }
    ...
    def get_memory_config(cls):
        ...
        try:
            ...
            if available_memory > 30:
                max_memory = min(cls.MAX_MEMORY_GB, available_memory - 15)  # 15GB 여유 확보
            elif available_memory > 20:
                max_memory = available_memory * 0.5  # 50%만 사용
            else:
                max_memory = available_memory * 0.4  # 40%만 사용
    ...
    def get_data_config(cls):
        ...
        # 메모리 기반 동적 크기 조정 (매우 보수적으로)
        if memory_config['max_memory_gb'] > 25:
            train_size = cls.MAX_TRAIN_SIZE
            test_size = cls.MAX_TEST_SIZE
        elif memory_config['max_memory_gb'] > 15:
            train_size = int(cls.MAX_TRAIN_SIZE * 0.5)  # 50%
            test_size = int(cls.MAX_TEST_SIZE * 0.5)
        else:
    ...
    def get_safe_memory_limits(cls):
        ...
        try:
            ...
            # 안전한 메모리 사용량 계산 (더 보수적으로)
            if available_gb > 40:  # 64GB 시스템
                safe_limit = 15  # 15GB로 제한
            elif available_gb > 25:  # 32GB 시스템
                safe_limit = 10  # 10GB로 제한
            elif available_gb > 12:  # 16GB 시스템
                safe_limit = 6   # 6GB로 제한
            else:
                safe_limit = available_gb * 0.3  # 30%만 사용
            
            return {
                'total_memory_gb': total_gb,
                'available_memory_gb': available_gb,
                'safe_memory_limit_gb': safe_limit,
                'recommended_chunk_size': min(cls.CHUNK_SIZE, int(safe_limit * 8000)),
                'recommended_batch_size': min(cls.BATCH_SIZE_GPU, int(safe_limit * 50))
            }
        except ImportError:
            ...
            return {
                ...
                'safe_memory_limit_gb': 15.0,
                'recommended_chunk_size': 150000,
...
try:
    ...
    os.environ['OMP_NUM_THREADS'] = '4'  # CPU 코어 수 제한
