...
def apply_ctr_correction(predictions: np.ndarray, target_ctr: float = 0.0191) -> np.ndarray:
    ...
    try:
        ...
        logger.info(f"Current CTR: {current_ctr:.6f}, Target CTR: {target_ctr:.6f}")
        ...
        if abs(current_ctr - target_ctr) > 0.001:
            ...
            logger.info(f"CTR correction applied: {current_ctr:.6f} -> {final_ctr:.6f} (factor: {correction_factor:.4f})")
    ...
    try:
        ...
        if ensemble_used and ensemble_manager.final_ensemble and ensemble_manager.final_ensemble.is_fitted:
            ...
            try:
                ...
                valid_predictions = [p for p in base_predictions.values() if len(p) == len(X_test)]
                if valid_predictions:
                    predictions = np.mean(valid_predictions, axis=0)
                else:
