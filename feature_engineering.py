...
def test_feature_engineering():
    ...
    try:
        ...
        print(f"3. 데이터 로드 완료 - 학습: {train_sample.shape}, 테스트: {test_sample.shape}")
        
        # 기본 전처리
        train_processed = data_loader.basic_preprocessing(train_sample)
        test_processed = data_loader.basic_preprocessing(test_sample)
        ...
        print("5. 피처 엔지니어 초기화 완료")
        
        # 피처 생성 실행
        X_train, X_test = feature_engineer.create_all_features(
            train_processed, test_processed, target_col='clicked'
        )
        
        print(f"6. 피처 엔지니어링 완료!")
        print(f"   - 원본 피처 수: {train_processed.shape[1] - 1}")  # clicked 제외
        print(f"   - 최종 피처 수: {X_train.shape[1]}")
        print(f"   - 생성된 피처 수: {len(feature_engineer.generated_features)}")
        
        # 결과 요약
        summary = feature_engineer.get_feature_importance_summary()
        print("\n7. 요약 정보:")
        for key, value in summary.items():
            if key != 'generated_features':  # 너무 길어질 수 있으므로 제외
                print(f"   - {key}: {value}")
        
        # 데이터 타입 확인
        print("\n8. 최종 데이터 타입:")
        print(f"   - 수치형 컬럼: {len(X_train.select_dtypes(include=[np.number]).columns)}")
        print(f"   - 범주형 컬럼: {len(X_train.select_dtypes(include=['object', 'category']).columns)}")
        
        # 결측치 확인
        missing_count = X_train.isnull().sum().sum()
        print(f"   - 결측치 총 개수: {missing_count}")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("\n테스트 중 오류가 발생했습니다. 위의 오류 메시지를 확인해주세요.")